  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 12, marginBottom: "1.5rem" }}>
        {workerSummary.map(w => (
          <div key={w.workerId} style={{ ...S.card, borderLeft: `3px solid ${parseFloat(w.outstanding) > 0 ? "#A32D2D" : "#386D11"}` }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
              <div style={{ width: 32, height: 32, borderRadius: 50, background: "#EEEDFE", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 600, color: "#543AB7" }}>
                {(w.workerName.split(" ")[0].map(x => x[0]).join(" ")).slice(0, 2)}
              </div>
              <div style={{ fontWeight: 600, fontSize: 13 }}>{w.workerName}</div>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                {[["Earned", fmtRs(w.totalEarned), "#185FA5"], ["Paid", fmtRs(w.totalPaid), "#386D11"]].map(([l, v, c]) => (
                  <div key={l} style={{ background: "#f9f9f9", borderRadius: 6, padding: "6px 8px" }}>
            ...
            <div style={{ marginTop: 8, paddingTop: 8, borderTop: "1px solid #f0f0f0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <span style={{ fontSize: 14, fontWeight: 700, color: parseFloat(w.outstanding || 0) > 0 ? "#A32D2D" : "#386D11" }}>{fmtRs(w.outstanding)}</span>
            </div>
            <button
      ...
      <div style={{ ...S.card, marginTop: 8 }}>
            ...
            <tbody>
              {filtPayments.map(p => (
                <tr key={p.id}>
                  ...
                  <td style={{ ...S.td, fontWeight: 600, color: "#0F6E56" }}>{fnts(p.amount)}</td>
                  ...
                  <td style={{ ...S.td }}>
                    {role === "ADMIN" && (
                      <button onClick={() => save(() => api.deletePayment(p.id))}
                          style={{ background: "#CEEBE0", color: "#A32D2D", border: "none", borderRadius: 5, padding: "3px 8px", cursor: "pointer", fontSize: 12 }}>Del</button>
                    )}
                  </td>
                </tr>
              ))}
        ...
        {filtPayments.length > 0 && (
          <div style={{ marginTop: 12, padding: "10px 12px", background: "#f9f9f9", borderRadius: 8, display: "flex", justifyContent: "flex-end", gap: 24, fontSize: 13 }}>
            <span style={{ color: "#888" }}>Total Shown</span>
            <span style={{ fontWeight: 700, color: "#0F6E56" }}>{fnts(filtPayments.reduce((a, p) => a + parseFloat(p.amount), 0))}</span>
          </div>
        )}
      </div>
    </div>
