    ...
    return (
        <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.25rem" }}>
                ...
                <div style={{ display: "flex", gap:8}}>
                    ...
                    <button onClick={() => setModal({ type: "dispatch", data: { stockItemId: "", workerId: "", workTypeId: "", pricePerPiece: "", sentQty: "", sentDate: today() } })} style ={S.btnPri}>
                        <i className="ti ti-send" style={{ fontSize: 13 }} />Dispatch
                    </button>
                </div>
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(155px, 1fr))", gap: 12, marginBottom: "1.5rem" }}>
                ...
                <StatCard label="Sent to Workers"       value={fmt(dashboard.totalSent)}                sub="pieces dispatched"  color="blue"    icon="ti-send"  />
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
                <div style={S.card}>
                    <h3 style={{ margin: "0 0 1rem", fontSize: 12, fontWeight: 600, color:"#888", textTransform:"uppercase", letterSpacing:"0.05em" }}>Worker Outstanding</h3>
                      {(dashboard.workerSummary || []).length === 0
                          ? <p style={{ color:"#aaa", fontSize:13 }}>No active dispatches</p>
                          : (dashboard.workerSummary || []).map( w=> (
                        <div  key={w.workerId} style={{ display:"flex", alignItems: "center", gap: 8, marginBottom:10 }}>
                            <div style={{ width: 30, height:30, borderRadius:50, background:"#EEEDFE", display:"flex", alignItems:"center", justifyContent:"center", fontSize:11, fontWeight:600, color:"#534AB7", flexShrink:0 }}>
                ...
                <div style={S.card}>
                   <h3 style={{ margin: "0 0 1rem", fontSize: 12, fontWeight: 600, color:"#888", textTransform:"uppercase", letterSpacing:"0.05em" }}>Recent Dispatches</h3>
                         {(dashboard.recentDispatches || []).map(d => (
                             <div  key={d.id} style={{ display:"flex", alignItems: "center", justifyContent:"space-between", paddingBottom:8, marginBottom:8, borderBottom:"0.5px solid #f0f0f0"}}>
                                <div>
                                    <div style={{fontSize: 13, fontWeight: 500}}>{d.workerName}</div>
                                    <div style={{fontSize: 11, color:"#888"}}>{d.fabric} . {d.workTypeName} </div>
                                </div>
                                <div style={{textAlign:"right" }}>
                                    <div style={{ fontSize:13 }}>{d.sentQty} pcs</div>
